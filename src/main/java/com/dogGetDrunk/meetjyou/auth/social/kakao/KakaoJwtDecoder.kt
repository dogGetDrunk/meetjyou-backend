...
class KakaoJwtDecoder(
    ...
    override fun decode(token: String): Jwt {
        ...
        val signed = try {
            ...
            log.warn("Kakao JWT parsing failed", e)
        ...
        if (claims.issuer != props.issuer) {
            log.warn("Kakao JWT validation failed: invalid issuer. expected={}, actual={}", props.issuer, claims.issuer)
            throw InvalidJwtException()
        }

        // 2. Verifying audience
        if (!claims.audience.contains(props.clientId)) {
            log.warn("Kakao JWT validation failed: invalid audience. expected={}, actual={}", props.clientId, claims.audience)
            throw InvalidJwtException()
        }

        // 3. Verifying expiration
        if (claims.expirationTime.before(Date())) {
            log.warn("Kakao JWT validation failed: token expired. expirationTime={}", claims.expirationTime)
            throw InvalidJwtException(message = "Token has expired")
        }

        // 4. Verifying nonce
        if (claims.getStringClaim("nonce") != nonce) {
            log.warn("Kakao JWT validation failed: invalid nonce. expected={}, actual={}", nonce, claims.getStringClaim("nonce"))
            throw InvalidJwtException()
        }
        ...
        if (!signed.verify(verifier)) {
            log.warn("Kakao JWT signature verification failed")
            throw InvalidJwtException()
        }
