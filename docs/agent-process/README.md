...
5. **완료 보고 (L5)** — 답변 맨 위에 `result:` 줄 + 원장 표 + 증거 + "확인하지 못한 것". 게이트(L3)가 형식·최신성 확인

## 완료 게이트 동작 (L3)

- 발동 조건: 마지막 메시지의 완료 주장 (`result:` 줄, "완료했/됐/되었", "수정했습니다", "고쳤습니다", "통과했", "전체 통과", "done" 등)
- 항상 검사: 갭 프로토콜 (아래)
- `src/`가 `origin/main` 대비 변경됐을 때: 마지막 성공한 전체 테스트 마커의 `src/` 트리 해시 = 현재 `src/` 트리 해시
  - 해시: `record-full-test.py`가 테스트 직후 `worktree_tree_hash`의 `src` 서브트리를 기록 → 커밋·문서 수정은 무관, `src` 내용 변경·삭제·이름 변경은 무효화
  - mtime 기각 (#146): 삭제 파일의 변경 시각을 git index mtime으로 대신 → 커밋만 해도 증거 무효 (PR 게이트 G24와 같은 원인)
  - 해시 없는 옛 마커는 증거로 불인정 → 전체 테스트 1회 재실행 필요
- base 브랜치(`main`) 자체에서는 `origin/main`과 같거나 뒤처진 상태를 "변경 없음"으로 판정 — 머지 후 fast-forward한 main이 방금 머지된 PR의 변경을 미검증 작업으로 오판하던 문제(#146). `origin/main`보다 앞선 로컬 커밋은 그대로 검사
...
- **강제 (완료 게이트, 개수 기반)**
  ...
  - base 브랜치(`main`) 자체의 기준점: base 대비 diff가 항상 비므로(#146) **가장 이른 추궁 시각 직전의 main 본선(`--first-parent`) 커밋**을 base로 사용 (곁가지 커밋을 잡으면 옛 항목이 새 항목으로 세어짐, G32) → main에서 판정한 갭은 이후 머지된 PR의 gap-log 항목으로 충족 (G31). 추궁 기록이 없으면 기존 base 유지
  - 한계: 개수만 비교하므로 추궁 이후 들어온 **다른 갭의 항목**도 충족으로 셈 (작업 브랜치와 동일)
